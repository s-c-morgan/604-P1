...
# Create missingness heatmap for a site
make_missing_plot <- function(site_name) {
  sdf <- site_missing_df(site_name)
  
  # Simplify facet labels to show only numbers
  sdf <- sdf %>%
    mutate(
      PlateFacet_Simple = factor(paste0("Plate ", PlateColIdx), 
                                levels = paste0("Plate ", 1:plates_per_site)),
      RepFacet_Simple = factor(paste0("Rep ", RepRowIdx),
                              levels = paste0("Rep ", 1:reps_per_site))
    )
  
  ggplot(sdf, aes(x = Col_Num, y = Row_Num, fill = data_status)) +
    geom_tile(color = "grey30", linewidth = tile_border_width) +
    facet_grid(RepFacet_Simple ~ PlateFacet_Simple, switch = "y", drop = FALSE) +
    scale_fill_manual(
      values = c("Observed" = "white", "Missing" = "red"),
      name = "Data Status"
    ) +
    scale_x_continuous(breaks = NULL, expand = c(0, 0)) +
    scale_y_continuous(breaks = NULL, trans = "reverse", expand = c(0, 0)) +
    labs(
      title = paste0("Missingness — Site: ", site_name, " (4 Replicates × 7 Plates)"),
      subtitle = "Red = Missing, White = Observed",
      x = NULL, 
      y = NULL
    ) +
...
# Generate plots for selected sites
for (site in sites_to_plot) {
  print(make_missing_plot(site))
}
```
